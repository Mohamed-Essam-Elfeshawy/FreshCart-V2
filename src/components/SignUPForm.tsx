...
import { signupFormInputs, slideLogin } from "@/data";
import { ShoppingCart } from "@mui/icons-material";
import { useForm, SubmitHandler } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import { schemaSignup } from "@/schema/schema";
import { ISignupInput } from "@/interface";
...
function SignUPForm() {
  ...
  const renderFormSign = signupFormInputs.map((el, idx) =>
    el.type !== "password" ? (
      <FormControl key={idx}>
      ...
      <Box key={idx}>
        ...
        <FormControl sx={{ width: "100%" }}>
          ...
          />
          {errors[el.id] && (
            <FormHelperText error>{errors[el.id]?.message}</FormHelperText>
          )}
        </FormControl>{" "}
      </Box>
    )
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: "1.5rem", fontWeight: "600" }}>
            Sign up for FreshCart
          </Typography>
          <Typography sx={{ fontSize: ".875rem", color: "#71717a" }}>
            Your one-stop shop for home appliances and smart devices
          </Typography>
